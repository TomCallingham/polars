...
// use polars_io::hdf5::utils::infer_file_schema;
// use polars_io::hdf5::{CommentPrefix, Hdf5Encoding, NullValues};
...
// #[cfg(feature = "hdf5")] //Is this needed?
#[derive(Clone)]
pub struct LazyHdf5Reader<'a> {
    args: ScanArgsHdf5,
    path: PathBuf,
    paths: Arc<[PathBuf]>,
}
...
impl LazyHdf5Reader {
    fn new(path: PathBuf, args: ScanArgsHdf5) -> Self {
        Self {
            args,
            path,
            paths: Arc::new([]),
...
impl LazyFileListReader for LazyHdf5Reader {
    ...
    fn finish_no_glob(self) -> PolarsResult<LazyFrame> {
        ...
        let mut lf: LazyFrame = LogicalPlanBuilder::scan_hdf5(
            paths,
            self.args.subgroup,
...
impl LazyFrame {
    /// Create a LazyFrame directly from a hdf5 scan.
    pub fn scan_hdf5(path: impl AsRef<Path>, args: ScanArgsHdf5) -> PolarsResult<Self> {
        LazyHdf5Reader::new(path.as_ref().to_owned(), args).finish()
    }

    /// Create a LazyFrame directly from a hdf5 scan.
    pub fn scan_hdf5_files(paths: Arc<[PathBuf]>, args: ScanArgsHdf5) -> PolarsResult<Self> {
        LazyHdf5Reader::new(PathBuf::new(), args)
            .with_paths(paths)
            .finish()
    }
}
